...
public class _21_parts1 {
 private static  String PREF_CSV_PATH = "csvFilePath";
 private static final String AVERAGE_TURNOVER_FILE = "2ndCsvFileG.txt";
 private static final String TURNOVER_FILE = "TurnoverG.txt";
...
public _21_parts1(JFrame parentFrame) {
 prefs = Preferences.userRoot().node(this.getClass().getName());
 frame = new JFrame("parts Data");
...
System.out.println("DEBUG: Added category: Unknown for brand: " + brand);
        ...
        if (!isUpdate && brandCategories.values().stream().flatMap(List::stream).anyMatch(category -> category.equalsIgnoreCase("parts"))) {
...
private Map<String, Integer> findHeaderIndices(Iterator<CSVRecord> records) {
 ...
 private Map<String, Integer> calculateBrandOccurrences() {
   Map<String, Integer> brandOccurrences = new HashMap<>();
   try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("TurnoverG.txt")))) {
       String line;
...
private Map<String, List<Double>> loadAveragePrices() {
  ...
  try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("2ndCsvFileG.txt")))) {
      String line;
      while ((line = reader.readLine()) != null) {
          String[] parts = line.split(",");
if (parts.length >= 5) {
...
calculateAndDisplayAverages(storedTurnoverTimes);
 ...
 private void saveItemData(Map<String, List<Double>> brandPrices, Map<String, List<String>> brandItemNumbers, Map<String, List<String>> brandSaleDates, Map<String, List<String>> brandCategories) {
  Set<String> processedItemNumbers = new HashSet<>();
  try (BufferedWriter writer = new BufferedWriter(new FileWriter(getAppDataFilePath("2ndCsvFileG.txt")))) {
      for (Map.Entry<String, List<Double>> entry : brandPrices.entrySet()) {
          ...
          for (int i = 0; i < prices.size(); i++) {
              ...
              if ("parts".equalsIgnoreCase(category) && !processedItemNumbers.contains(itemNumber)) {
...
/*
         ...
         */
  }else {JOptionPane.showMessageDialog(frame, "Selected category is not parts: " + category);
  
 }}
 public static String getCSVPath() {
     Preferences prefs = Preferences.userRoot().node(_21_parts1.class.getName());
