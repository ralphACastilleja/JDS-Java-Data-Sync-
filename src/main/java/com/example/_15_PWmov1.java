...
public class _15_PWmov1 {
 private static  String PREF_CSV_PATH = "csvFilePath";
 private static final String AVERAGE_TURNOVER_FILE = "2ndCsvFileD.txt";
 private static final String TURNOVER_FILE = "TurnoverD.txt";
...
public _15_PWmov1(JFrame parentFrame) {
 prefs = Preferences.userRoot().node(this.getClass().getName());
 frame = new JFrame("Pocket watch movement Data");
...
System.out.println("DEBUG: Added category: Unknown for brand: " + brand);
        ...
        if (!isUpdate && brandCategories.values().stream().flatMap(List::stream).anyMatch(category -> category.equalsIgnoreCase("Pocket Watch Movement"))) {
...
private Map<String, Integer> findHeaderIndices(Iterator<CSVRecord> records) {
 ...
 private Map<String, Integer> calculateBrandOccurrences() {
   Map<String, Integer> brandOccurrences = new HashMap<>();
   try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("TurnoverD.txt")))) {
       String line;
...
private Map<String, List<Double>> loadAveragePrices() {
  ...
  try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("2ndCsvFileD.txt")))) {
      String line;
      while ((line = reader.readLine()) != null) {
          String[] parts = line.split(",");
if (parts.length >= 5) {
...
calculateAndDisplayAverages(storedTurnoverTimes);
 ...
 private void saveItemData(Map<String, List<Double>> brandPrices, Map<String, List<String>> brandItemNumbers, Map<String, List<String>> brandSaleDates, Map<String, List<String>> brandCategories) {
  Set<String> processedItemNumbers = new HashSet<>();
  try (BufferedWriter writer = new BufferedWriter(new FileWriter(getAppDataFilePath("2ndCsvFileD.txt")))) {
      for (Map.Entry<String, List<Double>> entry : brandPrices.entrySet()) {
          ...
          for (int i = 0; i < prices.size(); i++) {
              ...
              if ("Pocket Watch Movement".equalsIgnoreCase(category) && !processedItemNumbers.contains(itemNumber)) {
...
List<String> categories = _09_BrandList.getCategories();
 ...
 private void displayDataForCategory(String category) {
     if ("Pocket Watch Movement".equalsIgnoreCase(category)) {
         _15_PWmov1.createEbayGUI(frame);
        
/*
     } else if (("pocket watch".equalsIgnoreCase(category))){
         _13_pocketWCAT1.createEbayGUI(frame);
     } else if (("Pocket Watch Movement Movement".equalsIgnoreCase(category))){
         ...
         */
  }else {JOptionPane.showMessageDialog(frame, "Selected category is not Pocket Watch Movement: " + category);
  
 }}
 public static String getCSVPath() {
     Preferences prefs = Preferences.userRoot().node(_15_PWmov1.class.getName());
