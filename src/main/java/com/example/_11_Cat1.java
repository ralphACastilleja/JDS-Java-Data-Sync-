...
public class _11_Cat1 {
 ...
 private static final String AVERAGE_TURNOVER_FILE = "2ndCsvFileB.txt";
 private static final String TURNOVER_FILE = "TurnoverB.txt";
...
public _11_Cat1(JFrame parentFrame) {
 prefs = Preferences.userRoot().node(this.getClass().getName());
 frame = new JFrame("WristWatch Data");
...
System.out.println("DEBUG: Added category: Unknown for brand: " + brand);
        ...
        if (!isUpdate && brandCategories.values().stream().flatMap(List::stream).anyMatch(category -> category.equalsIgnoreCase("Wristwatch"))) {
...
private Map<String, Integer> findHeaderIndices(Iterator<CSVRecord> records) {
 ...
 private Map<String, Integer> calculateBrandOccurrences() {
   Map<String, Integer> brandOccurrences = new HashMap<>();
   try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("TurnoverB.txt")))) {
       String line;
...
private Map<String, List<Double>> loadAveragePrices() {
  ...
  try (BufferedReader reader = new BufferedReader(new FileReader(getAppDataFilePath("2ndCsvFileB.txt")))) {
      String line;
      while ((line = reader.readLine()) != null) {
          String[] parts = line.split(",");
if (parts.length >= 5) {
...
calculateAndDisplayAverages(storedTurnoverTimes);
 ...
 private void saveItemData(Map<String, List<Double>> brandPrices, Map<String, List<String>> brandItemNumbers, Map<String, List<String>> brandSaleDates, Map<String, List<String>> brandCategories) {
  Set<String> processedItemNumbers = new HashSet<>();
  try (BufferedWriter writer = new BufferedWriter(new FileWriter(getAppDataFilePath("2ndCsvFileB.txt")))) {
      for (Map.Entry<String, List<Double>> entry : brandPrices.entrySet()) {
          ...
          for (int i = 0; i < prices.size(); i++) {
              ...
              if ("Wristwatch".equalsIgnoreCase(category) && !processedItemNumbers.contains(itemNumber)) {
...
/*
         ...
         */
  }else {JOptionPane.showMessageDialog(frame, "Selected category is not Wristwatch: " + category);
  
 }}
 public static String getCSVPath() {
     Preferences prefs = Preferences.userRoot().node(_11_Cat1.class.getName());
