...
public class _10_Tcalc {
    ...
    private static final String START_DATE_FILE = getAppDataFilePath("2ndCsvFile.txt");
    private static final String TURNOVER_FILE = getAppDataFilePath("Turnover.txt");


    //Finds the turnover time By looking at the completed orders text file, then the lisitngs text file, then crossrefrencing for corresponding item numbers 
    //Then calcauting the turnovertime by looking at the start data dn teh sale date 
    ...
    //Loads Item data from our files 
    ...
    //Gets the catagory from looking at the title 
    ...
    //Displays the data, caclautes average turnover tiem per brand by only looking at the brand, not title 
